...
log_path = os.path.join('snapshots_pick', 'test_close_open_hkhsi_%s.log'%ts)
log = open(log_path, 'w')
...
def get_data_label_dates(path, reverse=True):
    ...
    for i in range(len(dates) - slide_window - dayn):
        ...
        label.append([1,0] if targets2[i + slide_window + dayn] - targets1[i + slide_window] > 0 else [0,1])
